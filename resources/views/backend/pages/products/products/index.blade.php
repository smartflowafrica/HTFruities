...
@section('contents')
                            ...
                            <div class="tt-action">
                                ...
                                @can('delete_products')
                                    <button class="btn btn-danger" id="bulk-delete-btn" style="display: none;">
                                        <i data-feather="trash-2"></i> {{ localize('Delete Selected') }}
                                    </button>
                                @endcan
                            </div>
                        </div>
                        ...
                        <table class="table tt-footable border-top" data-use-parent-width="true">
                            <thead>
                                <tr>
                                    ...
                                    <th data-breakpoints="xs sm md">{{ localize('Is Featured') }}</th>
                                ...
                                @foreach ($products as $key => $product)
                                    <tr>
                                        <td class="text-center">
                                            <div class="form-check form-switch">
                                                <input class="form-check-input check-one" type="checkbox" name="ids[]" value="{{ $product->id }}">
                                            </div>
                                        </td>
                                        <td class="text-center">
                                        ...
                                        <td>
                                            <div class="form-check form-switch">
                                                <input type="checkbox" onchange="updateIsFesatured(this)"
                                                    class="form-check-input"
                                                ...
                                                <div class="dropdown-menu dropdown-menu-end shadow">
                                                    @can('edit_products')
                                                        <a class="dropdown-item"
                                                            href="{{ route('admin.products.edit', ['id' => $product->id, 'lang_key' => config('app.locale')]) }}&localize">
    ...
    <form id="bulk-delete-form" action="{{ route('admin.products.deleteAll') }}" method="POST" style="display: none;">
        @csrf
    </form>
@endsection
...
@section('scripts')
    <script>
        ...
        $(document).on('change', '.check-one', function() {
            var allChecked = $('.check-one:checked').length == $('.check-one').length;
        ...
        $('#bulk-delete-btn').on('click', function() {
            if(confirm('{{ localize("Are you sure you want to delete selected products?") }}')) {
                 var form = $('#bulk-delete-form');
                 form.html('@csrf'); // clear previous inputs but keep csrf
        ...
        function updateIsFesatured(el) {
            ...
            $.post('{{ route('admin.products.updateFeatureStatus') }}', {
                ...
                function(data) {
                    if (data == 1) {
                        notifyMe('success', '{{ localize('Feature product updated successfully') }}');
                    } else {
