...
if (!function_exists('staticAsset')) {
    ...
    {
        if (str_contains(url('/'), '.test') || str_contains(url('/'), 'http://127.0.0.1:') || App::environment('production')) {
...
if (!function_exists('renderStarRatingFrontOrganic')) {
    # render ratings frontend
...
if (!function_exists('formatPrice')) {
...

    {
        ...
        if (request()->hasHeader('Currency-Code')) {
            ...
            $price = floatval($price) * floatval(ApiCurrencyMiddleWare::currencyData()?->rate ?? 0);
    ...
    function getSubTotal($carts, $couponDiscount = true, $couponCode = '', $addTax = true)
    {
        ...
        if (count($carts) > 0) {
            foreach ($carts as $cart) {
                if ($cart->price > 0) {
                    $price += (float) $cart->price * $cart->qty;
                } else {
...
if(!function_exists('CountProductsByCategoryAndThemeId'))
{
    function CountProductsByCategoryAndThemeId($themeId, $categoryId )
    {
        ...
        $query = ProductTheme::selectRaw("count(products.id) as total_product")
            ...
            ->join("products", "products.id", "=", "product_themes.product_id")
