...
class ProductsController extends Controller
{
    ...
    public function edit(Request $request, $id)
    {
        ...
        // dd($lang_key);
        // http://grostore-laravel.test/admin/products/update-product/45?lang_key=en
    ...
    public function deleteAll(Request $request)
    {
        ...
        if (!$ids || count($ids) == 0) {
            flash(localize('Please select at least one product'))->error();
            return back();
        }
        ...
        try {
            ...
            foreach ($ids as $id) {
                ...
                if ($product) {
                    // Force delete by removing cart items first
                    if (!$product->carts->isEmpty()) {
            ...
            flash(localize($count . ' products have been deleted successfully'))->success();
        } catch (\Throwable $th) {
            flash(localize('Something went wrong during deletion: ' . $th->getMessage()))->error();
        }
