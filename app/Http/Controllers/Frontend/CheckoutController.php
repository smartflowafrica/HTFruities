    ...
    public function complete(Request $request)
    {
        ...
        try {
            ...
            if (count($carts) > 0) {
                ...
                if (session('is_bulk_order')) {
                    $orderGroup->is_bulk = 1;
                    session()->forget('is_bulk_order');
                }
                ...
                foreach ($carts as $cart) {
                    ...
                    // Custom Parfait Logic: If cart has custom data, save it and use custom price
                    if ($cart->data) {
                        $orderItem->description = $cart->data;
                        // Use the custom price from the cart if available, otherwise fallback
                        if ($cart->price > 0) {
